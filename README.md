# It Beats (OpenMW)

Feel the heartbeat of the Red Mountain.

*For MWSE aternative, check out [Heartthrum](https://www.nexusmods.com/morrowind/mods/47178) by RedFurryDemon and OperatorJack.*

## Audible Red Mountain Heartbeat

Whenever you're in the Red Mountain region (effectively, inside the Ghostfence), you'll hear Lorkhan's heartbeat - distant, yet disturbing...

The closer you get to the Heart, the louder the heartbeat becomes.  
The mod differentiates five separate player locations:

1. **Akulakhan's Chamber** - current cell id: `Akulakhan's Chamber`
2. **Dagoth Ur, Facility Cavern** (the room where you first meet Dagoth Ur) - current cell id: `Dagoth Ur, Facility Cavern`
3. **The rest of Dagoth Ur (the citadel)** - current cell is interior and its id contains: `Dagoth Ur`
4. **Generic interior** - current cell is an interior and at least one door leads to a Red Mountain region exterior cell (connected interiors are checked the same way)
5. **Exterior** - current cell is exterior and located in the Red Mountain region

Each location has a separate volume config and can be assigned different SFX.

After you deal with the Heart, the heartbeat - obviously - stops.

Heartthrum SFX are also included and available in the mod options.

## Compatibility

It's completely safe to install, update or remove it mid-playthrough.

## Consider Also

- [Red Mountain Tremors OpenMW](https://www.nexusmods.com/morrowind/mods/53637) by AbbaddoN33 and Demanufacturer87
- [Dynamic Ambient Visual Effects](https://www.nexusmods.com/morrowind/mods/55572) by taitechnic

## Credits

Sosnoviy Bor - Scripting  
MADpixel: SFX mixing  
RedFurryDemon and OperatorJack: Inspiration ([Heartthrum](https://www.nexusmods.com/morrowind/mods/47178))

Samples used:

- Heartthrum SFX by [Fredrik Jonasson](https://fredrikjonasson.net/) and RedFurryDemon
- [Thudding Heartbeat](https://pixabay.com/sound-effects/thudding-heartbeat-372487/) by DRAGON-STUDIO
- [024592_Earthquake remix of Laumark's Freesound #415125](https://pixabay.com/sound-effects/024592-earthquake-remix-of-laumark39s-freesound-415125flac-69744/) by freesound community
